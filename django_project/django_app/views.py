...
def tes_page(request):
    return render(request, 'django_app/tes_page.html')
...
def speech_to_text_process(request, pk):  # zidane
    ...
    text = Text(audio=text_audio, title=text_title, aplicants_name=aplicants_name,
                full_text="Audio belum di proses", length="3:33 min")
    text.save()
...
def speech_to_text_edit(request, pk):
    ...
    if request.method == 'POST':
        ...
        # ini ngambil value dari <input name="nama-file"> di html, trs disimpen ke tabel audio atributenya title
        text.full_text = request.POST['full_text']
...
def audio_upload(request):
    ...
    if request.method == 'POST':
        ...
        audio = Audio(title=audio_name, aplicants_name="Admin (Hard Coded)",
                      directory=audio_url, size=audio_size)
        audio.save()
...
def text_grading_score(request, pk):
    ...
    grade = Grade(text=text_tabel_text, a_score=Action, b_score=Imagine, c_score=Fokus, d_score=Integrity,
                  e_score=Enthusiasm, f_score=Totaly, g_score=Solid, h_score=Speed, i_score=Smart, final_score=final_score)
    grade.save()
...
def edit_audio_upload(request, pk):
    ...
    if request.method == 'POST':
        ...
        # ini ngambil value dari <input name="nama-pelamar"> di html, trs disimpen ke tabel audio atributenya aplicants_name
